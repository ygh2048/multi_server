...
void vMBUserRegInit(void)
{
    ...
    printf("[MB-REG] User registers initialized\r\n");
}
                            ...
                            USHORT usNRegs, eMBRegisterMode eMode)
{
    ...
    /* 检查地址是否在范围内：支持从0或1开始的地址 */
    usAddress = (usAddress == 0) ? 1 : usAddress;  /* 将地址0映射到1 */
        ...
        switch(eMode)
        {
            ...
            case MB_REG_WRITE:
                ...
                while(usNRegs > 0)
                {
                    ...
                    {
                        LED0 = (usRegHoldingBuf[iRegIndex] == 0) ? 1 : 0;
                    }
                    else if(iRegIndex == REG_LED2_ADDR)
                    {
                        LED1 = (usRegHoldingBuf[iRegIndex] == 0) ? 1 : 0;
                    }
    ...
    {
        eStatus = MB_ENOREG;
        printf("[MB-REG] ERROR: Invalid register address %u\r\n", (unsigned)usAddress);
    }
